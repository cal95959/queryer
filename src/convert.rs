...
// 因为rust trait的孤儿规则，如果想要对已有的类型实现已有的trait，需要简单包装一下
...
/// 把 SqlParser的BinaryOperator转换为DataFrame的Operator
...
/// 把 Sqlparser的SelectItem转换成DataFrame的 Expr
...
impl <'a> TryFrom<Source<'a>> for &'a str {
    ...
    fn try_from(value: Source<'a>) -> Result<Self, Self::Error> {
        ...
        if !table.joins.is_empty() {
            return Err(anyhow!("We don't support joint data source at the moment"));
        }
...
/// 把 SqlParser的offset expr 转换成 i64
...
/// 把 SqlParser 的Limit expr 转换成usize
...
/// 把 SqlParser 的 value转换成 DataFrame支持的LiteralValue
...
mod tests {
    ...
    fn parse_sql_works() {
        ...
        let statement = &Parser::parse_sql(&CalDialect::default(), sql.as_ref()).unwrap()[0];
